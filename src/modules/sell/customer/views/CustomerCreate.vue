...
const submit = async () => {
  ...
  const data = await customerStore.actionCreateCustomer(customerForm.value)
  if (data.status == 201) {
...
<template>
  <div class="customer-create flex justify-center">
    <div class="form-content">
      <h1 class="uppercase">{{ $t('Create_a_Customer') }}</h1>
      <form class="flex flex-col gap-[50px]" @submit.prevent="submit">
        ...
        <div class="actions flex w-full justify-end gap-4">
          <UIButton :disabled="isSubmitting">{{ $t('Create') }}</UIButton>
          <UIButton
            type="button"
            types="cancel"
