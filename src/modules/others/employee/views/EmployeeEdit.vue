...
const submit = async () => {
  ...
  employeeForm.value.phone_number = employeeForm.value.phone
  const data = await employeeStore.actionUpdateEmployee(employeeForm.value)
...
onMounted(async () => {
  ...
  selectedRole.value = roles.value.data.filter(
    (role) => role.id === employee.value.role?.data?.id
  )?.[0]
  employeeForm.value = {
    ...employeeForm.value,
    ...employee.value,
    role_id: selectedRole.value?.id,
    phone: employee.value.phone_number,
  }
})
</script>
...
<template>
  <div class="employee flex justify-center">
    <div class="form-content">
      <h1 class="uppercase">{{ $t('Edit_Employee') }}</h1>
