...
const removeTariff = (index: number) => {
  ...
  selectedTariffs.value.splice(index, 1)
}
...
const submit = async () => {
  ...
  if (productForm.value.product_image !== product.value.product_image) {
    formData.append('product_image', productForm.value.product_image)
  }
  formData.append('name', productForm.value.name)
  formData.append('_method', 'put')
  formData.append('category_id', selectedCategory.value.id.toString())
  ...
  selectedTariffs.value
    .map((item, index) => {
      return {
        tariff_id: item.id,
        price: productForm.value.tariffs?.[index].price.replace(',', '.'),
      }
    })
  ...
  const data = await productStore.actionUpdateProduct(
    formData,
...
onMounted(async () => {
  ...
  productForm.value.category_id = product.value.category_id
  productForm.value.id = product.value.id
  productForm.value.name = product.value.name
  productForm.value.pack_usage = product.value.pack_usage
  productForm.value.packs_quantity_per_pallet =
    product.value.packs_quantity_per_pallet
  productForm.value.price_per_pack = formatCurrency(
    product.value.price_per_pack,
    true,
    true
  )
  productForm.value.product_cost = formatCurrency(
    product.value.product_cost,
    true,
    true
  )
  productForm.value.product_image = product.value.product_image
  productForm.value.tariffs = product.value.productTariffs.data.map((item) => {
    return {
      tariff_id: item.tariff_id,
      price: formatCurrency(item.price, true, true),
    }
  })
  productForm.value.limits = product.value.product_limits_per_pack.map(
    (item) => item.limit
  )
  productForm.value.related_products = product.value.related_products
  selectedCategory.value = product.value.category.data
  selectedPack.value = PACK_USAGE.filter(
    (item) => item.value === String(product.value.pack_usage)
  )?.[0]
...
<template>
  <div class="provider-create flex justify-center">
    <div class="form-content">
      <h1 class="uppercase">{{ $t('Edit_Product') }}</h1>
      <div class="form flex flex-col gap-[50px]">
        <div class="flex gap-[30px]">
          <div class="upload-container">
            <UIImageInput
              @upload="setImage"
              :justPick="$route.name === 'Product'"
              :dataImage="
                productForm.product_image
                  ? getPhotoUrl(productForm.product_image)
                  : productForm.product_image
              "
